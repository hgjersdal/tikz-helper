...
(defmacro latex-environ ((ostream environ &optional (args nil)) &body body)
  "Place tex code generated by body within a latex environment"
  (if (null args)
      `(progn
	 (format ,ostream "\\begin{~a}~%" ,environ)
	 ,@body
	 (format ,ostream "\\end{~a}~%" ,environ))
      `(progn
	 (format ,ostream "\\begin{~a}[~a]~%" ,environ ,args)
	 ,@body
	 (format ,ostream "\\end{~a}~%" ,environ))))
...
(defmacro scope ((plottingarea &optional (style "")) &body body)
  "Make a tikz scope."
  `(latex-environ ((ostream ,plottingarea) "scope" ,style)
     ,@body))
